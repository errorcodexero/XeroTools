...
namespace PathViewer
{
    ...
    public class RobotPath
    {
        ...
        public void UpdateMaxVelocity(double oldv, double newv)
        {
            if (Math.Abs(oldv - MaxVelocity) < kEpsilon)
                MaxVelocity = newv;

            foreach(WayPoint pt in Points)
            {
                if (Math.Abs(pt.Velocity - oldv) < kEpsilon)
                    pt.Velocity = newv;
            }
        }

        public void UpdateMaxAcceleration(double oldv, double newv)
        {
            if (Math.Abs(oldv - MaxAcceleration) < kEpsilon)
                MaxAcceleration = newv;
        }

        public void UpdateMaxJerk(double oldv, double newv)
        {
            if (Math.Abs(oldv - MaxJerk) < kEpsilon)
                MaxJerk = newv;
        }
